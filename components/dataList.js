export default [
  {
    id: 0,
    searchKey: "viral-fever",
    sick: "Viral Fever",
    image:
      "https://s3.ap-southeast-1.amazonaws.com/images.deccanchronicle.com/dc-Cover-q4pm8e3cu6jvjkglehjef9lvd3-20170619023343.Medi.jpeg",
    goodFoods: {
      vegan: [
        {
          name: "Coriander Tea",
          desc: ""
        },
        {
          name: "Eat Raw Garlic & Onion",
          desc: ""
        },
        {
          name: "Honey, Lemon and Ginger Juice",
          desc: ""
        },
        {
          name: "Tulsi(Basil) Leaves",
          desc: ""
        }
      ],
      nonVegan: [
        {
          name: "Bland Diet",
          desc: ""
        },
        {
          name: "Gargle With Salt Water",
          desc: ""
        }
      ]
    }
  },
  {
    id: 1,
    searchKey: "headache",
    sick: "Headache",
    image:
      "https://cdn1.medicalnewstoday.com/content/images/articles/073/73936/headache.jpg",
    goodFoods: {
      vegan: [
        {
          name: "Russet Potato",
          desc: ""
        },
        {
          name: "Cucumber",
          desc: ""
        },
        {
          name: "Cherries",
          desc: ""
        },
        {
          name: "Fortified Whole Grain Cereal",
          desc: ""
        },
        {
          name: "Hot Peppers",
          desc: ""
        },
        {
          name: "Pumpkin Seeds",
          desc: ""
        },
        {
          name: "Oatmeal",
          desc: ""
        },
        {
          name: "Brown Rice",
          desc: ""
        }
      ],
      nonVegan: []
    }
  },
  {
    id: 2,
    searchKey: "flu",
    sick: "Flu",
    image:
      "https://cdn.shopify.com/s/files/1/0470/7525/articles/flu_1024x1024.jpg",
    goodFoods: {
      vegan: [
        {
          name: "Bananas",
          desc: ""
        },
        {
          name: "Rice",
          desc: ""
        },
        {
          name: "Applesauce",
          desc: ""
        },
        {
          name: "Toast",
          desc: ""
        },
        {
          name: "Flat ginger ale",
          desc: ""
        },
        {
          name: "Coke",
          desc: ""
        },
        {
          name: "Saltines",
          desc: ""
        }
      ],
      nonVegan: []
    }
  },
  {
    id: 3,

    searchKey: "common-cold",
    sick: "Common Cold",
    image:
      "https://www.findatopdoc.com/var/fatd/storage/images/healthy-living/the-common-cold-symptoms-and-management/67599-1-eng-US/The-Common-Cold-Symptoms-and-Management_article_main.jpg",
    goodFoods: {
      vegan: [
        {
          name: "Broths",
          desc: ""
        },
        {
          name: "Garlic",
          desc: ""
        },
        {
          name: "Coconut Water",
          desc: ""
        },
        {
          name: "Hot Tea",
          desc: ""
        },
        {
          name: "Honey",
          desc: ""
        },
        {
          name: "Ginger",
          desc: ""
        },
        {
          name: "Spicy Foods",
          desc: ""
        },
        {
          name: "Bananas",
          desc: ""
        },
        {
          name: "Oatmeal",
          desc: ""
        },
        {
          name: "Yogurt",
          desc: ""
        },
        {
          name: "Avocados",
          desc: ""
        }
      ],
      nonVegan: [
        {
          name: "Chicken Soup",
          desc: ""
        }
      ]
    }
  },
  {
    id: 4,
    searchKey: "hurts-to-swallow",
    sick: "It hurts to swallow",
    image:
      "https://cdn1.medicalnewstoday.com/content/images/articles/320/320621/throat-ulcers.jpg",
    goodFoods: {
      vegan: [
        {
          name: "Combine peppermint tea",
          desc: "(lukewarm, not hot) which has analgesic and anesthetic effects"
        },
        {
          name: "Manuka honey",
          desc: "Known for its wound-healing properties"
        },
        {
          name: "Soft creamy foods such as cream soups",
          desc: ""
        },
        {
          name: "Mashed Potatoes",
          desc: ""
        },
        {
          name: "Yogurt",
          desc: ""
        },
        {
          name: "Custards",
          desc: "Also soothing"
        }
      ],
      nonVegan: [
        {
          name: "Scrambled eggs",
          desc: ""
        }
      ]
    }
  },
  {
    id: 5,
    searchKey: "runny-nose",
    sick: "Runny nose",
    image:
      "https://www.thespruceeats.com/thmb/hh7q_4n69WWWKpHxIAgRao_oatA=/2048x1365/filters:no_upscale():max_bytes(150000):strip_icc()/homemade-ginger-tea-3377239-5-5_preview-5afd9c53c06471003692829d.jpeg",
    goodFoods: {
      vegan: [
        {
          name: "Wakaya ginger tea",
          desc:
            "Suggests Slayton. Ginger contains antioxidant and anti-inflammatory properties that may help clear up a cold faster than just waiting it out"
        }
      ],
      nonVegan: []
    }
  },
  {
    id: 6,
    searchKey: "gastritis",
    sick: "Gastritis",
    image:
      "https://www.seasonsmedical.com/wp-content/uploads/Gastritis-1024x860.jpeg",
    goodFoods: {
      vegan: [
        { name: "apples", desc: "Try high-fiber food" },
        { name: "oatmeal", desc: "Try high-fiber food" },
        { name: "carrots", desc: "Try high-fiber food" },
        { name: "broccoli", desc: "Try high-fiber food" },
        { name: "beans", desc: "Try high-fiber food" },
        {
          name: "vegetables",
          desc: "foods with low acidity, or more like alkaline"
        }
      ],
      nonVegan: [
        {
          name: "fish",
          desc: "Try low-fat food"
        },
        {
          name: "chicken",
          desc: "Try low-fat food"
        },
        {
          name: "turkey breast",
          desc: "Try low-fat food"
        }
      ]
    }
  },
  {
    id: 7,
    searchKey: "stress",
    sick: "Stress",
    image:
      "https://www.helpguide.org/images/stress/young-woman-fingers-to-temples-500.jpg",
    goodFoods: {
      vegan: [
        { name: "tea", desc: "It calms frazzled nerves" },
        {
          name: "Wholee-Wheat Pretzels",
          desc: ""
        },
        {
          name: "Carrots",
          desc: ""
        },
        {
          name: "Milk",
          desc: ""
        },
        {
          name: "Yogurt",
          desc: ""
        },
        {
          name: "Nut",
          desc: ""
        }
      ],
      nonVegan: [
        {
          name: "Fish",
          desc: ""
        }
      ]
    }
  },
  {
    id: 8,
    searchKey: "high-blood-pressure",
    sick: "High blood pressure",
    image:
      "https://cdn.images.express.co.uk/img/dynamic/11/590x/high-blood-pressure-symptom-stroke-diet-supplement-956307.jpg",
    goodFoods: {
      vegan: [
        {
          name: "Leafy greens",
          desc:
            "high in potassium. Potassium helps your kidneys get rid of more sodium through your urine. This in turn lowers your blood pressure"
        },
        { name: "Red beets", desc: "" },
        { name: "Skim milk", desc: "" },
        { name: "yoghurt", desc: "" },
        { name: "Oatmeal", desc: "" },
        { name: "Bananas", desc: "" }
      ],
      nonVegan: [
        { name: "Salmon", desc: "" },
        { name: "mackerel", desc: "" },
        {
          name: "fish",
          desc: "fish with omega-3s also good for high blood pressure"
        }
      ]
    }
  },
  {
    id: 9,
    searchKey: "constipation",
    sick: "Constipation",
    image:
      "https://cdn.24.co.za/files/Cms/General/d/6736/4edcc78735b94f1dbb4040820fd71419.png",
    goodFoods: {
      vegan: [
        {
          name: "prunes( Dried plums)",
          desc: "rich in fibers"
        },
        {
          name: "Apples",
          desc: "rich in fibers"
        },
        {
          name: "Pears",
          desc: "rich in fibers"
        },
        {
          name: "Kiwifruit",
          desc: "rich in fibers"
        },
        {
          name: "Artichokes",
          desc:
            "it have a prebiotic effect, promoting good gut health and regularity"
        },
        {
          name: "Rhubarb",
          desc:
            "a leafy plant that is well known for its bowel-stimulating properties"
        }
      ],
      nonVegan: []
    }
  },
  {
    id: 10,
    searchKey: "diabetes",
    sick: "Diabetes",
    image:
      "https://rotasaude.lusiadas.pt/wp-content/uploads/2015/11/tipos-de-diabetes.jpg",
    goodFoods: {
      vegan: [
        { name: "avocado", desc: "it can help regulate appetite" },
        {
          name: "sliced mushrooms",
          desc:
            "Use as a meat alternative in recipes like stir fries for a way to fill up without the added calories"
        }
      ],
      nonVegan: []
    }
  },
  {
    id: 11,
    searchKey: "menstrual-cramps",
    sick: "Menstrual Cramps",
    image:
      "https://cdn2.stylecraze.com/wp-content/uploads/2014/07/Top-10-Foods-To-Relieve-Menstrual-Cramps-1.jpg",
    goodFoods: {
      vegan: [
        {
          name: "Chamomile Tea",
          desc: "relief abdominal cramp and calm you down"
        },
        {
          name: "Ginger Tea",
          desc: "relief abdominal cramp"
        },
        {
          name: "Dark Leafy Vegetables",
          desc:
            "full of iron, which helps when you are losing iron during menstruation"
        }
      ],
      nonVegan: [
        {
          name: "Salmon",
          desc: "Omega-3 will reduce imflammation and pain"
        }
      ]
    }
  },

   {
    id: 12,
    searchKey: "heart-disease",
    sick: "Heart Disease",
    image:
      "https://secure.i.telegraph.co.uk/multimedia/archive/02943/meddiet_2943573b.jpg",
    goodFoods: {
      vegan: [
        {
          name: "Legumes and Soy",
          desc: "oy protein has been shown to lower LDL cholesterol levels, especially if blood cholesterol levels are high"
        },
        {
          name: "Tea",
          desc: "some evidence suggests that the antioxidants in tea can help prevent the build-up of fatty deposits in the arteries."
        },
        {
          name: "Nuts and Seeds",
          desc:
            "hey should be eaten in small quantities, as they are high in kilojoules"
        },
        {
          name: "Some vegetables oils ",
          desc: "such as corn, soy and safflower, which contain omega-6 fatty acids, and those containing omega-3 fatty acids such as canola and olive oil."
        },
        {
          name: "Garlic",
          desc: "a compound in fresh garlic called allicin has been found in some studies to lower blood cholesterol"
        },
        {
          name:"Fods enriched with plant sterols",
          desc:" daily intake of 2–3 g of phytosterols/stanols lowers LDL cholesterol levels by approximately ten per cent in healthy people and in those with high cholesterol and those with diabetes. This intake can be achieved by the consumption of two to three serves of phytosterol-enriched foods like margarine spreads, reduced-fat yoghurts, milk and breakfast cereals."
        }
      ],
      nonVegan: [
        {
          name: "Oily Fish",
          desc: "such as mackerel, sardines, tuna and salmon which contain omega-3 fatty acids"
        }
      ]
    }
  },
  {
    id: 13,
    searchKey: "dengue",
    sick: "Dengue",
    image:"https://www.kailashhealthcare.com/UserFiles/HealthTips/429cb41b-89d4-408f-a636-f90f54e9854a.jpg",
    goodFoods: {
      vegan: [
        {
          name: "Papaya Leaves",
          desc: "Just crush the papaya leaves and squeeze them to extract the juice"
        },
        {
          name: "Broccoli",
          desc: "Broccoli is an excellent source of Vitamin K which helps to regenerate blood platelets."
        },
        {
          name: "Pomegranate",
          desc:"Pomegranate is rich in essential nutrients and minerals that provides the body with required energy."
        }
      ]
    }
  }
];
