...
- Because `routes` is defined explicitly, Vercel does **not** auto-generate routes for the Next.js builder's static assets (the "opted out of @vercel/next's optimized lambdas mode" build warning is telling you this). `vercel.json` must include an explicit `/_next/(.*)` rule ahead of the catch-all `/(.*)` rule, or the JS/CSS bundles 404 and the whole site fails to hydrate (looks fine via SSR, nothing is interactive).
- `www`'s own static assets (currently `app.css`, `nprogress.css`, referenced from `_document.js`) must live in `www/public/` and be referenced at the site root (e.g. `/app.css`), not `www/static/`. Next.js deprecated the `static/` folder years ago; the local custom `server.js` still serves it via Next's own request handler (so this can silently work in local dev/prod testing), but `@vercel/next`'s deployment packaging follows Next's own asset manifest, which no longer includes `static/` — files placed there 404 on the actual Vercel deployment even if `vercel.json` has a route rule pointing at them. This exact gap once broke the search dropdown's CSS (`.react-autosuggest__*` positioning rules in `app.css`) in production while everything looked fine locally.
